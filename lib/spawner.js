...
function spawnServer(server) {
	...
	runningServers[server.port].ffmpeg = child_process.spawn(ffmpeg, ["-f", "video4linux2", "-i", server.device, "-r", config.rate, "-acodec", "none", "-s", config.width+"x"+config.height, "http://localhost:" + server.port.toString() + "/v4lweb_" + server.port.toString() + ".ffm"], env);
