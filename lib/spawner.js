...
function spawnServer(server) {
	...
	runningServers[server.port].ffmpeg = child_process.spawn(ffmpeg, ["-f", "video4linux2", "-i", server.device, "-r", "30", "-acodec", "none", "-s", "640x480", "http://localhost:" + server.port.toString() + "/v4lweb_" + server.port.toString() + ".ffm"], env);
